import { createFile, getDownloadFileSignedURL } from "wasp/client/operations";
...
export async function uploadFileWithProgress({
  ...
  const { s3UploadUrl, s3UploadFields, newFile } = await createFile({
    fileType: file.type,
    fileName: file.name,
  });
  ...
  axios.post(s3UploadUrl, formData, {
    onUploadProgress: (progressEvent) => {
      if (progressEvent.total) {
        ...
        if (percentage == 100) setUploadProgressPercent(0)
      }
    },
  });
  const newFileUrl = takeFileUrl ?  await getDownloadFileSignedURL({key: newFile.key}) : undefined
  return {createdFile: newFile, createdFileUrl: newFileUrl };
}
